...
def quadratic(batch_size=128, num_dims=10, stddev=0.01, dtype=tf.float32, problems_w=None,
              problems_b=None):
  ...
  def build():
    ...
    if problems_w is None:
      ...
      w = tf.get_variable("w",
                          shape=[batch_size, num_dims, num_dims],
                          dtype=dtype,
                          initializer=tf.constant_initializer(problems_w),
                          trainable=False)
      y = tf.get_variable("y",
                          shape=[batch_size, num_dims],
                          dtype=dtype,
                          initializer=tf.constant_initializer(problems_b),
                          trainable=False)
...
def prob_sin(batch_size=128, num_dims=2, stddev=0.01, dtype=tf.float32, problem_param=None):
  # def quadratic(batch_size=1, num_dims=10, stddev=0.01, dtype=tf.float32):
  """f(x, y) = np.sin(x + b) ** 5 + a * np.cos(b + y * x) * np.cos(c * x)"""
